...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
/********************************/
/* See: Definition of mediaplay */
/********************************/
static const char* extension_tbl[] = {
    ".mp3",
    ".wma",
    ".m4a",
    ".wav",
...
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
...
MTPDevice::MTPDevice():
    ...
    m_root_dir(),
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    m_dirfetchcontent_path(),
    m_dirfetchcontent_dir(nullptr),
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
...
#if 0 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
bool MTPDevice::connect_priv(int dev_no, const std::string &dev_file)
#else /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
bool MTPDevice::connect_priv(int dev_no, int bus_node, int dev_node, const std::string &dev_file)
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
{
    if (m_device) {
        logerr("Already connected.\n");
...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    if (dev_no < 0 && bus_node != -1 && dev_node != -1) {
        for (int i = 0; i < raw_devices_cnt; i++) {
            if (bus_node == (int)raw_devices[i].bus_location && dev_node == (int)raw_devices[i].devnum) {
            dev_no = i;
            break;
            }
        }
    ...
    if (!enumStorages()) {
        ...
        // wait for STORE_ADDED event
        while(1) {
            if (LIBMTP_Read_Event(m_device, &event, &param) != 0) {
                return false;
            }
...
#else /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    return connect_priv(dev_no, -1, -1, std::string());
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
}
...
#else /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    return connect_priv(-1, -1, -1, dev_file);
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
}
#endif

#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
bool MTPDevice::connect(int bus_node, int dev_node)
{
    return connect_priv(-1, bus_node, dev_node, std::string());
}
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
...
{
    uint64_t total = 0;
#if 0  /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
    for (LIBMTP_devicestorage_t *s = m_device->storage; s; s = s->next)
        total += s->MaxCapacity;
#else  /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
    if (m_device) {
        for (LIBMTP_devicestorage_t *s = m_device->storage; s; s = s->next)
            total += s->MaxCapacity;
    }
#endif /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
    return total;
}
...
#else /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
const TypeDir *MTPDevice::dirFetchContent(const std::string &path)
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
{
#if 0  /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
    if (!m_root_dir.isFetched()) {
#else  /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
    if (!m_root_dir.isFetched() && m_device) {
#endif /* _PIDST_MODIFIED_STATIC_CODE_ANALYSIS_ */
        for (LIBMTP_devicestorage_t *s = m_device->storage; s; s = s->next) {
...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    if (path == m_dirfetchcontent_path)
        return m_dirfetchcontent_dir;
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
int MTPDevice::fileRead(const std::string &path, char *buf, size_t size, off_t offset)
{
    const std::string src_basename(smtpfs_basename(path));
    const std::string src_dirname(smtpfs_dirname(path));
    const TypeDir *dir_parent = dirFetchContent(src_dirname);
    const TypeFile *file_to_fetch = dir_parent ? dir_parent->file(src_basename) : nullptr;
    if (!dir_parent || !file_to_fetch) {
        logerr("No such file '", path, "' to read.\n");
        return -EINVAL;
    }

	/* Correct the size. (Offset + size) value does not exceed file size. */
	uint64_t	ui_filesize = file_to_fetch->size();
	if (static_cast<uint64_t>(offset + size) > ui_filesize){
		size = ui_filesize - offset;
	}

    unsigned int read_size = 0;
    char* temp_mem = new char(size);
    if (!temp_mem) {
        return -EINVAL;
    }

    int ret = LIBMTP_GetPartialObject(m_device, file_to_fetch->id(), offset, size, (unsigned char**)&temp_mem, &read_size);
    if (0 != ret) {
        delete[] temp_mem;
        return -EINVAL;
    }
...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
bool MTPDevice::isMediaFile(const LIBMTP_file_t &file)
{
    char *p_extension = rindex(file.filename, '.');
    if (p_extension) {
        for (int i=0; extension_tbl[i]; i++) {
            if (!strcasecmp(p_extension, extension_tbl[i])) {
                return true;
            }
        }
    }
