...
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
        SMTPFS_OPT_KEY("-b %i", m_bus_node, -1),
        SMTPFS_OPT_KEY("--busnode %i", m_bus_node, -1),
        SMTPFS_OPT_KEY("-d %i", m_dev_node, -1),
        SMTPFS_OPT_KEY("--devnode %i", m_dev_node, -1),
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
        FUSE_OPT_END
    };
...
bool SMTPFileSystem::exec()
{
    ...
    if (m_options.m_device_file) {
        ...
        if (!m_device.connect(m_options.m_device_file))
            return false;
#if 1 /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    } else if (m_options.m_bus_node != -1 && m_options.m_dev_node != -1) {
        if (!m_device.connect(m_options.m_bus_node, m_options.m_dev_node))
            return false;
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    } else {
    ...
    off_t offset, struct fuse_file_info *file_info)
{
    ...
    const std::string std_path(path);
    int rval = m_device.fileRead(std_path, buf, size, offset);
#endif /* _PIDST_MODIFIED_SIMPLEMTPFS_ */
    return rval;
}
